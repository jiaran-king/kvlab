...
async def get_prefiller_info(prefill_clients: list, ready: asyncio.Event):
    for prefill_client in prefill_clients:
        ...
        while time.monotonic() < deadline:
            ...
            raise TimeoutError("Prefill bootstrap readiness exceeded 1800s")
...
def log_phase(event, request_id, **fields):
...
async def send_request_to_service(
    ...
    log_phase("P_http_start", request_id)
    try:
        response = await client_info["client"].post(
            endpoint, json=req_data, headers=headers
        )
        log_phase("P_http_response", request_id, status=response.status_code,
                  upstream_request_id=response.headers.get("x-request-id"))
        response.raise_for_status()
    except Exception as exc:
        log_phase("P_http_error", request_id, error_type=type(exc).__name__, error=str(exc))
        raise
...
async def stream_service_response(
    ...
    async with decode_client_info["client"].stream(
        ...
        log_phase("D_http_headers", request_id, status=response.status_code,
                  upstream_request_id=response.headers.get("x-request-id"))
        response.raise_for_status()
        first = True
        async for chunk in response.aiter_bytes():
            if first:
                log_phase("D_first_chunk", request_id)
                first = False
            yield chunk
        log_phase("D_stream_end", request_id)
...
async def _handle_completions(api: str, request: Request):
    ...
    try:
        ...
        log_phase("proxy_received", request_id, seed=req_data.get("seed"),
                  requested_max_tokens=req_data.get("max_tokens"),
                  requested_max_completion_tokens=req_data.get("max_completion_tokens"))
