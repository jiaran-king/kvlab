...
for d in Path('/proc').iterdir():
 ...
 try:
  ...
  os.kill(int(d.name),signal.SIGTERM);row['action']='one bounded SIGTERM to exact known Codex reconnect residue; not claimed reaped';out['known_residues'].append(row)
