...
def emit(kind,**fields):
 ...
 if _rows>=20000:raise RuntimeError('query diagnostic row limit reached')
 if _file is None:
...
def instrument(module):
 if module.__name__.endswith('kv_cache_manager'):
  ...
  def lookup(self,request,*args,**kwargs):
   t=time.perf_counter();out=get(self,request,*args,**kwargs)
   row=dict(request_id=request.request_id,num_tokens=request.num_tokens,num_prompt_tokens=request.num_prompt_tokens,num_computed_tokens=request.num_computed_tokens,preemptions=request.num_preemptions,hit_tokens=out[1],lookup_seconds=time.perf_counter()-t,allocation=None)
