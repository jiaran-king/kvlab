...
try:
    ...
    for role in ['P','D']:
        launch(['srun','--exclusive','--exact','--nodes=1','--ntasks=1','--cpus-per-task=16','--mem=256G','--gres=gpu:h20:2','--time=01:55:00',str(root/'runtime/bin/python'),str(base/'stage-v4/diagnostic-12g/pd_service.py'),str(root),str(run),role],role)
    ...
    replay=launch([str(root/'client/bin/python'),str(base/'stage-v4/diagnostic-12g/replay_v4.py'),str(base),str(run),label],'replay')
