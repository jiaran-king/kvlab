...
try:
    ...
    for role in ['P','D']:
        launch(['srun','--exclusive','--exact','--nodes=1','--ntasks=1','--cpus-per-task=16','--mem=256G','--gres=gpu:h20:2','--time=01:40:00',str(root/'runtime/bin/python'),str(base/'stage-v4/pd_service.py'),str(root),str(run),role],role)
