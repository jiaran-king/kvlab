...
def check_smoke(run):
 ...
 committed={(x['request_id'],x['attempt']) for e in ev if e['kind']=='query_batch_committed' for x in e['attempts']}
 assert all((e['request_id'],e['attempt']) in committed for e in dec), 'Query batch not committed'
...
def check_formal(run):
 ...
 baseline=json.loads((run.parents[2]/'stage-v4/formal-3430/replay/replay-plan.json').read_text())
 ...
 for rid,es in dec.items():
  ...
  committed={(x['request_id'],x['attempt']) for v in ev if v['kind']=='query_batch_committed' for x in v['attempts']}
  for e in es:
   assert (rid,e['attempt']) in committed
