...
def instrument(module):
 ...
 if module.__name__.endswith('kv_cache_manager'):
  ...
  def schedule(self,*args,**kwargs):
   ...
   if _pending:
    emit('query_batch_committed',attempts=[{'request_id':r['request_id'],'attempt':r['attempt']} for r in _pending])
    _file.flush()  # Complete the query batch before returning scheduling decisions.
   _pending.clear()
