...
try:
    ...
    for role in ['P','D']:
        launch(['srun','--exclusive','--exact','--nodes=1','--ntasks=1','--cpus-per-task=16','--mem=256G','--gres=gpu:h20:2','--time=01:55:00',str(root/'runtime/bin/python'),str(base/'stage-v6/same-capacity-12g/pd_service.py'),str(root),str(run),role],role)
    ...
    if os.environ.get('FROZEN_RUN'):
        warmup=launch([str(root/'runtime/bin/python'),str(base/'stage-v6/same-capacity-12g/frozen_warmup.py'),os.environ['FROZEN_RUN'],str(run)],'warmup')
    else:
    ...
    wait_idle()
    subprocess.run([str(root/'runtime/bin/python'),str(base/'stage-v6/same-capacity-12g/check_evidence.py'),'smoke',str(run)],check=True,timeout=60)
    probes=list(run.glob('query-diagnostic-*.jsonl'))
    assert any('"kind": "lookup"' in p.read_text() for p in probes), 'P diagnostic hook did not observe warmup lookups'
    ...
    replay=launch([str(root/'client/bin/python'),str(base/'stage-v6/same-capacity-12g/replay_v4.py'),str(base),str(run),label],'replay')
